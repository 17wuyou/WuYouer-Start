...
def prepare_dataloaders(config):
    ...
    train_data = data[:train_end]; val_data = data[train_end:val_end]; test_data = data[val_end:]
    ...
    x_val = torch.from_numpy(x_val).float(); y_val = torch.from_numpy(y_val).float()
    batch_size = config['train']['batch_size']
    train_dataset = TensorDataset(x_train, y_train)
    val_dataset = TensorDataset(x_val, y_val)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    # 返回一个空的test_loader，因为这个函数目前不生成它
...
def main(config_path):
    ...
    if os.path.exists(checkpoint_path):
        try:
            ...
            logging.info(f"Checkpoint found. Resuming training from epoch {start_epoch}.")
        except Exception as e:
            logging.warning(f"Could not load checkpoint. Starting from scratch. Error: {e}")
            # 如果检查点文件损坏，确保相关目录存在，以便重新开始
            os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
    else:
        logging.info("No checkpoint found. Starting training from scratch.")
        # 确保检查点目录存在，以便保存第一个检查点
        os.makedirs(os.path.dirname(checkpoint_path), exist_ok=True)
    # --- 自动化逻辑结束 ---
